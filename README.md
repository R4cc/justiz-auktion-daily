...
The Admin page also offers token grants. An admin can select one existing user and add a whole amount from 1 to 1,000,000 tokens to that account, or credit every account that exists at submission time, including admin accounts. Each grant is a single atomic transaction with a saved receipt and retry protection. Future registrations receive their normal starting balance but never inherit previous bulk grants. The page shows current user balances, the bulk recipient count, and the latest 20 receipts of each kind. Grants do not consume or reset daily game rewards.
...
The repository carries a flag-gated technical foundation for the coming auction economy: market categories with a global per-category index, a news-event store with structural market effects, player resale auctions of inventory items (with item locking, so a listed item cannot be sold or listed twice), a "Mystery Palette" vocabulary over the existing case system, and sealed primary Mystery Palette auctions (HTTP reads, authenticated bidding and winner-only reward reveal as thin adapters over the finished domain). None of it changes gameplay, and every system stays invisible until enabled: set `FEATURE_NEWS`, `FEATURE_MARKET`, `FEATURE_RESALES`, `FEATURE_PALETTES`, or `FEATURE_PALETTE_AUCTIONS` to `1` to expose the corresponding endpoints (resale mutations and admin news seeding ride along behind the first two flags; `FEATURE_PALETTES` is the catalog read model only, while `FEATURE_PALETTE_AUCTIONS` separately gates actual auction acquisition — enabling palettes alone never exposes auctions). See [`docs/auction-economy-foundation.md`](docs/auction-economy-foundation.md) for the schema, API contracts, compatibility decisions, and the explicit list of intentionally unfinished mechanics.
