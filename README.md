...
Each newly created account starts with **J€ 1,000**. Existing accounts are not modified by this setting. One rewarded run is available per **UTC day**, shared between Daily and Higher or Lower:
...
Collectible resale values follow their auction values directly: **one euro of auction value becomes J€ 1**, rounded to whole J€ with a minimum of J€ 1. This applies to existing unsold collectibles as well as new finds. Each case price is calculated from the probability-weighted average resale value of that edition's contents and targets a 94.25% average J€ return, rounded to whole J€.
...
Businesses are available at `/businesses`. Wine, toy, electronics, and car shops each have four sizes, chosen in the Buy store dialog. Street pop-ups, small, medium, and large shops hold 10, 25, 60, and 150 items respectively; car dealerships hold 1, 3, 8, and 20 vehicles. The street pop-up starts at J€350 (J€875 for cars); larger locations cost more and draw more visitors. Items must match the shop type. NPC direct auctions replenish eight bulk lots every two hours, and winning lots settle into individual inventory units. Bids reserve J€ immediately; outbid players receive exact refunds. Store customers are simulated hourly on the server, including elapsed hours while the site is offline. Stock sells for 30% over its market-adjusted value (at least J€1 above it), and each sale credits the owner once. Unsold stock can be removed from a store and used elsewhere.
