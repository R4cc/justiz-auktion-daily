...
function renderHigherLower(focus = false) {
  ...
  app.innerHTML = `<section class="hl-screen">
    ...
    ${run.revealed ? `<div class="hl-verdict"><h2>${!run.correct ? t('Game over', 'Vorbei') : finished ? t('Finished', 'Fertig') : tied ? t('Tie', 'Gleichstand') : t('Correct', 'Richtig')}</h2>${finished ? `<p>${run.streak} ${t('correct', 'richtig')}</p>` : ''}</div>
      <button class="primary-button" data-action="${finished ? 'higher-lower' : 'hl-next'}">${finished ? t('Play again ↻', 'Noch einmal spielen ↻') : t('Next lot →', 'Nächstes Los →')}</button>` : `<div class="hl-choices"><button class="primary-button" data-action="hl-higher" ${run.pending ? 'disabled' : ''}>↑ ${t('Higher', 'Höher')}</button><button class="secondary-button" data-action="hl-lower" ${run.pending ? 'disabled' : ''}>↓ ${t('Lower', 'Niedriger')}</button></div>`}
    <button class="hl-home" data-action="home">${t('Back to home', 'Zur Startseite')}</button>
    </div><div class="data-note"><span>${run.index} / ${run.auctions.length - 1}</span> ${infoTip(t('Ties accept either choice. One miss ends the run. Prices are confirmed final bids from different lots.', 'Bei Gleichstand zählen beide Tipps. Ein Fehler beendet den Lauf. Die Preise sind bestätigte Endgebote verschiedener Lose.'), t('Game rules', 'Spielregeln'))}</div>
  </section>`;
