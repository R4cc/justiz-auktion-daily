...
test('HTTP API enforces authentication, CSRF headers, admin permissions and session cookies without exposing secrets', async t => {
  ...
  assert.equal((await fetch(base + 'admin/auctions', { headers: { cookie: playerCookie } })).status, 403);
  const browserList = await (await fetch(base + 'admin/auctions', { headers: { cookie } })).json();
  assert.equal(browserList.total, lots.length);
  assert.equal(browserList.pageSize, 25);
  assert.ok(browserList.auctions.every(auction => !('description' in auction)));
  const searched = await (await fetch(base + 'admin/auctions?query=product%20100', { headers: { cookie } })).json();
  assert.equal(searched.total, 1);
  assert.equal(searched.auctions[0].id, lots[0].id);
  const detail = await (await fetch(base + `admin/auctions/${lots[3].id}`, { headers: { cookie } })).json();
  assert.equal(detail.auction.title, lots[3].title);
  assert.equal(detail.auction.correctPrice, lots[3].currentBid);
  assert.equal((await fetch(base + 'admin/auctions/999999', { headers: { cookie } })).status, 404);
  const paged = await (await fetch(base + 'admin/auctions?page=9', { headers: { cookie } })).json();
